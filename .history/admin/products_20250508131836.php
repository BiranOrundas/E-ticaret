...
<div class="content p-4">
  ...
  <div class="row g-4">
    ...
    <?php
// … yukarıda $editProd zaten çekilmiş olmalı …
?>
...
<form method="post" enctype="multipart/form-data">
  ...
  <div class="mb-3">
    <label class="form-label">Yeni Resimler (multiple)</label>
    <input type="file" name="images[]" class="form-control" multiple>
  </div>

  <?php if($editProd): ?>
    <div class="mb-3">
      <label class="form-label">Mevcut Resimler (vitrin için seç)</label>
        ...
        <?php foreach($imgs as $img): ?>
          <div class="text-center">
            <label style="cursor:pointer">
              <img src="../public/productimg/<?= htmlspecialchars($img['image_path']) ?>"
                   style="width:100px;height:100px;object-fit:cover;border:2px solid <?= $editProd['main_image_id']==$img['id']?'#007bff':'#ddd' ?>;">
              <div>
                <input type="radio" name="main_image_id" value="<?= $img['id'] ?>"
                  <?= $editProd['main_image_id']==$img['id']?'checked':'' ?>>
                Vitrin
              </div>
            </label>
          </div>
